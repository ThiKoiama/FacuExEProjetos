...
typedef struct Livro tLista; 


/* Cria um novo elemento, atribui o valor informado e retorna 
   o valor  */
tLista *Criar(int vcodigo, char vnome[30], int vclassificacao, float vpreco) {
	tLista *temp;
	temp = (tLista *) malloc(sizeof temp);
	
	temp->codigo = vcodigo;
    strcpy(temp->nome, vnome);
    temp->classificacao = vclassificacao;
    temp->preco = vpreco;
	temp->prox = NULL;
	temp->ante = NULL;
...
void InserirFinal(int vcodigo, char vnome[30], int vclassificacao, float vpreco, tLista *lista) {
	tLista *temp;
	...
	temp->prox = (Criar(vcodigo, vnome, vclassificacao,vpreco));
...
/* Percorre a lista do in�cio para o final exibindo os 
   valores*/
